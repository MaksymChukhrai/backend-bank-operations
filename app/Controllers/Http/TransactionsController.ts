...
export default class TransactionsController {
   ...
   */
  public async update({ params, request, response }: { params: {id: string}, request: HttpRequest, response: HttpResponse }) {
    ...
    const balanceChange = transaction.type === 'income' ? priceDifference : -priceDifference;
    ...
    const previousBalance = previousTransaction ? previousTransaction.balance_after : 0;
    transaction.balance_after = previousBalance + (transaction.type === 'income' ? newPrice : -newPrice);
    
    // Вместо Redis используем простое кеширование в памяти
    TransactionsController.cache[`transaction:${transaction.id}:balance`] = transaction.balance_after.toString();
    
    // Имитируем последующие транзакции
    const subsequentTransactions = [];
    for (let i = id + 1; i <= id + 3; i++) {
      subsequentTransactions.push(new Transaction({
        id: i,
    ...
    for (const nextTransaction of subsequentTransactions) {
      // Проверяем кеш
      const cachedBalance = TransactionsController.cache[`transaction:${nextTransaction.id}:balance`];
      ...
      if (cachedBalance) {
        ...
        currentBalance = currentBalance + (nextTransaction.type === 'income' ? nextTransaction.price : -nextTransaction.price);
      }
      
      nextTransaction.balance_after = currentBalance;
      
      // Обновляем кеш
      TransactionsController.cache[`transaction:${nextTransaction.id}:balance`] = currentBalance.toString();
    }
